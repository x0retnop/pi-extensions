...
validated end-to-end through pi (sub-agents runner invocation shape).

Final config: temperature 0.6, top_p 0.95, presence_penalty 1.5,
max_tokens 24576, enable_thinking true.
- t=1.0: vaguer, invented one function name. pp=0: slightly noisier numbers.
- t=0.6/pp=1.5: cleanest verbatim outcomes, quotes, commit hashes.
- Observed: ~11-13k prompt tokens -> 2-6k completion tokens, reasoning
  0.6-4.5k chars, 35-110s, finish_reason=stop. max_tokens is headroom.
- Run-to-run variance exists: one run writes ~15 condensed command rows,
  another ~110 exhaustive rows. Both are faithful; condensing is sampling
  luck, not config.

How params reach the model (verified):
- pi NEVER sends temperature/top_p/presence_penalty (no such fields in its
  code). It sends max_tokens (model.maxTokens) and thinking controls.
- Thinking: pi supports `--thinking <level>` and `--model provider/id:<level>`;
  for local Qwen use provider compat `thinkingFormat: "qwen-chat-template"`
  (maps to chat_template_kwargs.enable_thinking). The sub-agents runner does
  not pass --thinking; with qwen-chat-template the effective default keeps
  thinking ON (verified: reasoning blocks present in e2e output).
- Sampling params reach the model via the infer-gateway preset
  `sub-agents-handoff`, applied through a provider-level header in
...
- Fallback path: provider local-llama (:1234, raw llama-server) — no param
  control except server launch flags (server_CFG); prompt alone still gives
  decent results (run E: all hashes/quotes/errors preserved, less exhaustive).

History of this file: the original version had a Russian preamble BEFORE the
frontmatter, which broke parseFrontmatter (regex requires ^--- at line 1), so
loadAgentsFromDir skipped the agent entirely; `model: Qwen/Qwen3.5-9B` also
never resolved (no provider `Qwen` in models.json).

Note: /handoff in sub-agents/index.ts is hardcoded to `handoff-gemma`; this
agent runs via /sub-agents TUI (or change the hardcoded name in index.ts).
-->
