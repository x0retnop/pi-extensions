...
type CommandLike = ReturnType<ExtensionAPI["getCommands"]>[number] & {
  path?: string;
  sourceInfo?: { path?: string };
};

function getCommandPath(command: CommandLike, cwd: string): string {
  const rawPath = command.sourceInfo?.path ?? command.path ?? "";
  return rawPath ? normalizeReadPath(rawPath, cwd) : "";
}
...
function buildSkillIndex(pi: ExtensionAPI, cwd: string): SkillIndexEntry[] {
  return pi
    ...
    .map((c) => {
      const p = getCommandPath(c as CommandLike, cwd);
      return {
...
class ContextView implements Component {
  ...
  private rebuild(width: number): void {
    ...
    lines.push("");
    lines.push(
      muted("Session: ") + text(`${this.data.session.totalTokens.toLocaleString()} tokens`) + muted(" · ") + text(formatUsd(this.data.session.totalCost)),
    );
