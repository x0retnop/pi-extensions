...
export function buildCollapsedDiffHintText(
	...
	const hiddenHunksText = options.hiddenHunks > 0
		...
		: undefined;
	const shortRemainingText = `${options.remainingLines} ${pluralize(options.remainingLines, "diff line")}`;
	const shortHiddenHunksText = options.hiddenHunks > 0
		? `${options.hiddenHunks} ${pluralize(options.hiddenHunks, "hunk")}`
		: undefined;

	const candidates = [
		`… (${[shortRemainingText, shortHiddenHunksText, "Ctrl+O"].filter(Boolean).join(" • ")})`,
