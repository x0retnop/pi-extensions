...
|---|---|---|
| >= 0.80.4 | **Inline entries** | Dim row directly under each tool call in the scrollback: `MM-DD HH:MM:SS  tool (dur)  target`. Ctrl+O expands a ≤2-line result snippet (`↳`). Errors marked `✗`. |
...
- Snippet: first 2 non-empty text lines of the result, control chars stripped, ≤120 chars each. Persisted in the session file — keep it small.
